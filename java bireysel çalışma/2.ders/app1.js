/*
? Veri Tipleri

1-String HARF İÇEREN YERLERDE STRİNG TİPİ KULLANILIR ÇİFT TIRNAKLAR ARASINA ALINIR
2-Number
3-Boolean
4-Null
5-Undefined
6-Object
7-Function
*/

//----------------STRİNG VERİTİPİ-----------------
// let isim = "Enes Bayram 23 yaşındadır.";
// console.log(isim);
// console.log(typeof isim);

// let sayi1 = 10.7;
// let sayi2 = 5.3;

// console.log(sayi1+sayi2);
// console.log(typeof sayi1);

//----------------NUMBER VERİTİPİ-----------------

// 5 +2 = 7

// let a = "5";
// let b = "2";

// Yaşınız  : 23

// let stringDegisken  ="Yaşınız : ";
// let yas = 23;

// console.log(stringDegisken + yas);

//----------------BOOLEAN VERİTİPİ-----------------

//boolean : true ve false 'dan oluşur

// console.log(5>8);

// let a = 3;
// let b = 12;

// let sonuc = a+b;
// console.log(sonuc!=15);

//----------------NULL VERİTİPİ-----------------

// let a = null; // boş

// a=27;

// console.log(a);

//----------------UNDEFİNED VERİTİPİ-----------------

// let a;
// console.log(typeof a);

// primitive ve referans tipler nedir , farkları nelerdir ?

//----------------OBJECT VERİTİPİ-----------------
// let insan = {
//     isim : "Enes",
//     soyisim : "Bayram",
//     yas : 23
// }

// console.log(typeof insan);

// let rakamlar =[1,2,3,4,5];
// console.log(typeof rakamlar);

// let func = function(){
//     console.log("Merhaba");
// }

// func();
// console.log(typeof func);

let isim = "Göksal Karabulut 30 yaşındadır.";
console.log(isim);
let sayı1 = 15;
console.log(sayı1);
var isim1 = "göksal";
console.log(isim1);

let sayı11 = 45;
let sayı2 = 50;
console.log(sayı11 + sayı2);
console.log(sayı11 * sayı2);
console.log(sayı11 / sayı2);

//? ** operatörü sayının üssünü almak için kullanılır
let a = 6;
console.log(6 ** 3);
